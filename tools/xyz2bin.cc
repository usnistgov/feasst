...
int main(int argc, char** argv) {
  ...
  for (int p = 0; p < nProc; ++p) {
      ...
      if ( (space->natom() != 0) && ( (nMol == -1) || (space->nMol() == nMol) ) ) {
        if ( (!inFile.eof()) && (iter%skip==0) ) {
          ...
          if ( (beta < crit->mMax()) && (beta > crit->mMin()) ) { //catch out of bounds due to EOF in log
            ...
            // print the xyz files for a given bin
            int printFlag = 0;
            if (firstPrint[bin]) {
              printFlag = 1;
              firstPrint[bin] = false;
            }
            ss.str("");
            if (nProc == 1) {
              ss << ssFileIn.str() << "b" << bin;
            } else {
            ...
            pData.accumulate(bin, space->x(0,0));
          }
        }
      }
    ...
    for (int i = 0; i < pData.size(); ++i) {
      outFile << crit->bin2m(i) << " " << pData.vec()[i].average() << endl;
    }

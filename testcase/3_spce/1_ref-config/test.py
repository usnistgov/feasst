    ...
    def test(self):
        ...
        for srswConfig in range(1, 3):
            ...
            if srswConfig == 1:
                ...
                self.assertAlmostEqual(pair.peQReal()*KJperMol2Kelvin, -558888.91972785105, delta=feasst.DTOL)
                self.assertAlmostEqual(pair.peQFrr()*KJperMol2Kelvin, 6270.0937839397402, delta=feasst.DTOL)
                # Note, this is the negative of the sum of E_self and E_intra in SRSW
                self.assertAlmostEqual(pair.peQFrrSelf()*KJperMol2Kelvin, 34699.373693030466, delta=feasst.DTOL)
                self.assertAlmostEqual(pair.peTot()*KJperMol2Kelvin, -488603.17839517148, delta=feasst.DTOL)
            elif srswConfig == 2:
                ...
                self.assertAlmostEqual(pair.peQReal()*KJperMol2Kelvin, -1192948.6940245957, delta=feasst.DTOL)
                self.assertAlmostEqual(pair.peQFrr()*KJperMol2Kelvin, 6034.9503269097158, delta=feasst.DTOL)
                # Note, this is the negative of the sum of E_self and E_intra in SRSW
                self.assertAlmostEqual(pair.peQFrrSelf()*KJperMol2Kelvin, 69398.747386013289, feasst.DTOL)
                self.assertAlmostEqual(pair.peTot()*KJperMol2Kelvin, -1065894.9284973294, feasst.DTOL)

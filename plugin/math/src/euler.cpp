...
void Euler::compute_rotation_matrix(RotationMatrix * matrix) const {
  ...
  } else if (matrix->num_rows() == 2) {
    matrix->set_2d(phi_);
  } else {
    FATAL("unrecognized size of matrix:" << matrix->num_rows());
  }
}

void Euler::set(const Matrix& matrix) {
  const std::vector<std::vector<double> >& mat = matrix.matrix();
  if (matrix.num_rows() == 3 && matrix.num_columns() == 3) {
  ...
  } else if (matrix.num_rows() == 2 && matrix.num_columns() == 2) {
    phi_ = std::atan2(mat[1][0], mat[0][0]);
    theta_ = 0.;
    psi_ = 0.;
  } else {
    FATAL("unrecognized size");
  }
}
...
Euler::Euler(std::istream& istr) {
  ...
  ASSERT(version >= 3509 && version <= 3509, "unrecognized version: " << version);

...
TEST(Table1D, interpolate) {
  ...
  EXPECT_NEAR(table->linear_interpolation(0.5), 1./2., NEAR_ZERO);
  EXPECT_EQ(table->bin_to_value(1), 1);
  EXPECT_EQ(table->value_to_nearest_bin(0.45), 0);
  EXPECT_EQ(table->value_to_nearest_bin(0.501), 1);
}
...
TEST(Table2D, interpolate) {
  ...
  EXPECT_NEAR(table->linear_interpolation(0.5, 0.5), 1./4., NEAR_ZERO);

...
void Tune::serialize(std::ostream& ostr) const {
  ...
  feasst_serialize_version(257, ostr);
  feasst_serialize(trials_per_tune_, ostr);
}

Tune::Tune(std::istream& istr) : Modify(istr) {
  const int version = feasst_deserialize_version(istr);
  ASSERT(version >= 256 && version <= 257, "version mismatch:" << version);
  ...
  if (version <= 256) {
    ...
    *get_values_() = values_;
    *get_num_attempts_() = num_attempts_;
    *get_num_accepted_() = num_accepted_;
  }
}
...
void Tune::initialize(MonteCarlo * mc) {
  ...
  get_values_()->resize(num_trials);
  get_num_attempts_()->resize(num_trials, 0);
  get_num_accepted_()->resize(num_trials, 0);
  for (int trial = 0; trial < num_trials; ++trial) {
    if (tfac.trial(trial).num_stages() > 0) {
      (*get_values_())[trial] = tfac.trial(trial).stage(0).perturb().tunable().value();
    }
  }

...
TEST(VisitModelInnerRecursiveTable, serialize) {
  ...
  //auto vis = std::make_shared<VisitModelInnerRecursiveTable>(argtype({{"input_file", table_file}}));
  //vis->precompute(config.get());
  auto vis = std::make_shared<VisitModelInnerRecursiveTable>();

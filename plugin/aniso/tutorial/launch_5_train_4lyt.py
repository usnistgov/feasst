...
TODO:
1. build training data with Mayer of actualy rigid bodies. Parallelize.
2. build contact table. Parallelize. Compare
"""
...
def parse():
    ...
    parser.add_argument('--reference_sigma', type=float, default=30.,
    ...
    parser.add_argument('--hours_checkpoint', type=float, default=24, help='hours per checkpoint')
    parser.add_argument('--hours_terminate', type=float, default=24, help='hours until termination')
    ...
    params = vars(args)
    print('Warning, seed should default back to -1')
    params['script'] = __file__
    params['prefix'] = 'tr4lyt'
...
Configuration cubic_side_length=80 periodic=false,false,false \
    particle_type=t1:/feasst/plugin/aniso/tutorial/4lyt_aniso.txt,t2:/feasst/plugin/aniso/tutorial/4lyt_aniso.txt add_num_t1_particles=1 add_num_t2_particles=1 \
    group=fixed,mobile fixed_particle_index=0 mobile_particle_index=1 \
    cutoff=0 set_cutoff_min_to_sigma=true
WriteModelParams output_file={prefix}{sim:03d}_model_params.csv
Record save_positions={prefix}{sim:03d}_init.xyz
Potential Model=HardSphere VisitModel=VisitModelCell min_length=max_sigma energy_cutoff=1e50
RefPotential ref=hs Model=HardSphere sigma=0 cutoff=0 sigma0_33={reference_sigma} cutoff0_33={reference_sigma}
#HWH Had to manually set to number of types for distinct pair
#HWH Also had to manually set the cubic side length
ThermoParams beta={beta}

# initialize MayerSampling
MayerSampling trials_per_cycle={tpc} cycles_to_complete={equilibration_cycles}
TrialTranslate new_only=true ref=hs tunable_param=20 group=mobile
TrialRotate new_only=true ref=hs tunable_param=160 group=mobile
...
BuildRecursiveTable mayer_training_file={prefix}{sim:03d}_training.csv hard_limit_u={hard_limit_u} size={tabsize} num_orientations_per_pi={num_orientations_per_pi} beta=1 min_criteria=2220.22 output_file={prefix}{sim:03d}_table.txt
#cutoff=4.154700538
...
MonteCarlo
RandomMT19937 seed={seed}
Configuration particle_type=fluid:/feasst/plugin/aniso/particle/aniso_tabular.txt \
    cubic_side_length=10 add_num_fluid_particles=2 group=second second_particle_index=1
Potential Model=TwoBodyTable VisitModelInner=VisitModelInnerRecursiveTable input_file=0_0:{prefix}{sim:03d}_table.txt ignore_energy=true
...
def post_process(params):
    ...
    #assert np.abs(b2['second_virial_ratio']) < 0.15

...
#ifndef FEASST_CONFINEMENT_MODEL_TABLE_SPHERICAL_H_
#define FEASST_CONFINEMENT_MODEL_TABLE_SPHERICAL_H_
...
/**
  A tabular potential for interactions in one spherical dimension.
...
class ModelTableSphere1D : public ModelOneBody {
 public:
  ...
  /** @name Arguments
    ...
    - Sphere arguments.
   */
  explicit ModelTableSphere1D(argtype args = argtype());
  explicit ModelTableSphere1D(argtype * args);
  ...
  std::shared_ptr<Model> create(std::istream& istr) const override {
    return std::make_shared<ModelTableSphere1D>(istr); }
  std::shared_ptr<Model> create(argtype * args) const override {
    return std::make_shared<ModelTableSphere1D>(args); }
  explicit ModelTableSphere1D(std::istream& istr);
  virtual ~ModelTableSphere1D();
...
#endif  // FEASST_CONFINEMENT_MODEL_TABLE_SPHERICAL_H_

"""
This is a simple test of a ModelTableCylinder1D by computing the Henry
Coefficient of a particle in a cylinder.
"""
...
def parse():
    ...
    if args.plot_table == 1:
        ...
        for dist in dists:
            ...
            ens.append(df['ModelTableCylinder1D'].values[0])
        import matplotlib.pyplot as plt
...
Potential Model=ModelTableCylinder1D radius=3 first_point=0,0,0 second_point=0,0,1 table_file={prefix}_table.txt
...
def post_process(params):
    ...
    with open(params['prefix']+'.csv') as f:
        ...
        print(henry)
        assert np.abs(henry['beta_taylor'][0] - 0.129407) < 3*df['block_stdev'][0]

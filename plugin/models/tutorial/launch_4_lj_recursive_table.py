...
Then, RecursiveTablePotential is built with the training set.
"""
...
def parse():
    ...
    parser.add_argument('--rho', type=float, default=9e-3, help='highest number density')
    ...
    parser.add_argument('--table_file', type=str, default='ljr_table.txt', help='table file name')
    ...
    params['cubic_side_length'] = np.power(params['num_particles']/params['rho'], 1./3.).tolist()
    return params, args
...
Configuration cubic_side_length=1e10 periodic=false,false,false \
    particle_type=fluid:{fstprt} add_num_fluid_particles=2 \
    group=second second_particle_index=1
Potential Model=RecursiveTablePotential mayer_training_file={prefix}{sim:03d}_training.csv hard_limit_u=100 size=16 beta=1 min_criteria=0.05
RefPotential ref=hs Model=HardSphere sigma=0 sigmaLJ={reference_sigma} cutoff=0 cutoffLJ={reference_sigma}
ThermoParams beta={beta}
MayerSampling trials_per_cycle={tpc} cycles_to_complete={equilibration_cycles}
TrialTranslate new_only=true ref=hs tunable_param=1 group=second
...
pts = np.arange(0.75**2, 9, (9-0.75**2)/15)
#print('pts', pts)

def pltone(ax, df, xlim, ylim, color):
    ax.plot(np.sqrt(df[0]), df[3], color=color, label='1st')
    ax.plot(np.sqrt(df[0]), df[5], color=color, label=None, linestyle='dashed')
    ax.plot(np.sqrt(df[0]), df[2], color='black', label='LJ', linestyle='dotted')
    ax.set_ylim(ylim)
    ax.set_xlim(xlim)
    ax.scatter(np.sqrt(pts), 0*pts, color=color)
...
def post_process(params):
    ...
    print('lj:', b2lj)
    assert np.abs(b2lj['second_virial_ratio'] + 3.05) < 0.07
    print('lj table:', b2ljtable)
    assert np.abs(b2ljtable['second_virial_ratio'] + 2.9) < 0.07

    import matplotlib.pyplot as plt
    fig, axes = plt.subplots(4, 1, sharex=True)

    df1 = pd.read_csv("test0", header=None, delimiter=r"\s+")
    print(df1, df1[0])
    pltone(axes[0], df1, xlim=[0.9, 2], ylim=[-1, 1], color='blue')
    axes[0].set_ylabel(r'$U_1$', fontsize=16)

    df2 = pd.read_csv("test1", header=None, delimiter=r"\s+")
    pltone(axes[1], df2, xlim=[0.9, 2], ylim=[-1, 1], color='red')
    axes[1].set_ylabel(r'$U_2$', fontsize=16)

    df3 = pd.read_csv("test2", header=None, delimiter=r"\s+")
    pltone(axes[2], df3, xlim=[0.9, 2], ylim=[-1, 1], color='green')
    axes[2].set_ylabel(r'$U_3$', fontsize=16)

    df4 = pd.read_csv("test3", header=None, delimiter=r"\s+")
    pltone(axes[3], df4, xlim=[0.9, 2], ylim=[-1, 1], color='purple')

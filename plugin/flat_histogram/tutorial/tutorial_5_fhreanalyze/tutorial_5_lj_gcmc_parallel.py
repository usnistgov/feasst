...
def mc(thread, mn, mx):
    ...
    tm = fst.MakeTransitionMatrix(fst.args({"min_sweeps": str(args.min_sweeps)}))
    tm.dump_file("dump" + str(thread) + ".txt")
    mc.set(fst.MakeFlatHistogram(
        fst.MakeMacrostateNumParticles(
            fst.Histogram(fst.args({"width": "1", "max": str(mx), "min": str(mn)}))), tm))
    ...
    mc.add(fst.MakeTuner(fst.args({"steps_per": steps_per, "stop_after_phase": "0"})))

...
CollectionMatrixSplice min_window_size 2 hours_per {hours_per_adjust} ln_prob_file \
  trimer_lnpi.txt bounds_file trimer_bounds.txt num_adjust_per_write 10
...
python launch_08_trimer_tm_parallel.py --run_type 1 --task $SLURM_ARRAY_TASK_ID
if [ $? == 0 ]; then
  echo "Job is done"
...
class TestFlatHistogramTrimer(unittest.TestCase):
    def test(self):
        import numpy as np
        import pandas as pd
#        lnpi=pd.read_csv('trimer_lnpi.txt')

...
PARSER.add_argument('--temperature', type=float, default=270, help='temperature in Kelvin')
...
particle_type 0 weight 2 transfer true site 0 num_steps 4 reference_index 0
bond true mobile_site 1 anchor_site 0 num_steps 4 reference_index 0
...
def post_process(params):
    ...
    print('vapor density(kg/m)', vapor.average_macrostate()*dens_conv)
    print('liquid density(kg/m)', liquid.average_macrostate()*dens_conv)
    assert np.abs(295 - vapor.average_macrostate()*dens_conv) < 10
    assert np.abs(621 - liquid.average_macrostate()*dens_conv) < 50

...
TEST(CollectionMatrix, serialize) {
  ...
  TripleBandedCollectionMatrix colmat2 = test_serialize(colmat);
  EXPECT_DOUBLE_EQ(colmat2.matrix()[0][0], 0.2);
  ...
  TripleBandedCollectionMatrix colmat3 = test_serialize(colmat2);
...
TEST(TripleBandedCollectionMatrix, blocks) {
  auto cm = MakeTripleBandedCollectionMatrix();
  cm->resize(6);
  ...
  TripleBandedCollectionMatrix colmat2 = test_serialize(*cm);

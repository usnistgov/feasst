"""
Example Mayer-sampling (https://doi.org/10.1103/PhysRevLett.92.220601) simulation of a 2D Kern-Frenkel square well (https://doi.org/10.1063/1.1569473) with a hard circle reference.
The Janus particle has a 90 degree patch angle, with a coverage, chi = 0.5.
"""
...
Configuration side_length=1e10,1e10 periodic=false,false \
    ...
    group=mobile,centers mobile_particle_index=1 centers_site_type=A \
...
def post_process(params):
    ...
    b2reduced_analytical = 1-0.25*(np.power(params['cutoff'], 2)-1)*(np.exp(params['beta'])-1)

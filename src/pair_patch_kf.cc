...
PairPatchKF::PairPatchKF(Space *space, const argtype &args)
  ...
  // parse patchAngle
  initPatchAngle(stod(argparse_.key("patchAngle").str()));

  argparse_.checkAllArgsUsed();
}

void PairPatchKF::initPatchAngle(const double angle) {
  patchAngle_ = angle;
...
void PairPatchKF::setOrder(const double order) {
  Pair::setOrder(order);

  if (orderName_.compare("patch_angle") == 0) {
    initPatchAngle(order);
  }
}

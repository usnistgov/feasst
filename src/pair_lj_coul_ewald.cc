...
void PairLJCoulEwald::defaultConstruction_() {
  ...
  chargeConversion_ = 1./sqrt(4.*PI*permitivity);
}
...
double PairLJCoulEwald::allPartEnerForce(const int flag) {
  ...
  peLJone_ = 0;
  peQRealone_ = 0;
//  const double netCharge = netCharge_();
//  peQRealone_ = (netCharge*netCharge)*(-peRecCorrPrefac_*4.*PI/space_->volume()
//                 + alpha/sqrt(PI));
  ...
  // add the slab correction
  peQRealone_ += slab_correction_();
...
//  /**
//   * Augmentation due to net charge.
//   *
//   * For deletion (flag==2), qnew and qold and "switched"; however,
//   * this is corrected by the "minus" sign incorporated into the
//   * change in energy of the contribution of mpart upon deletion.
//   */
//  if ( (flag == 2 || flag == 3) ) {
//    const double qnew = netCharge_();
//    const double qold = qnew - netCharge_(mpart);
//    peQRealone_ = (qnew*qnew - qold*qold)*
//      (-peRecCorrPrefac_*4.*PI/space_->volume() + alpha/sqrt(PI));
//  }
  ...
  // add the slab correction
  peQRealone_ += slab_correction_(mpart);

  return peLJone_ + peLRCone_ + peQRealone_ + peQFrrone_;
}

double PairLJCoulEwald::slab_correction_(const std::vector<int>& mpart) const {
  double en = 0.;
  if (slab_corr_dim_ >= 0) {
    const std::vector<int>& type = space()->type();
    // compute the dim-direction dipole moment
    // for both mpart and those not in mpart
    double dipole_in = 0., dipole_out = 0.;
    for (int iAtom = 0; iAtom < space()->natom(); ++iAtom) {
      const double dipole = q_[type[iAtom]]*space()->x(iAtom, slab_corr_dim_);
      if (space()->natom() == static_cast<int>(mpart.size())) {
        dipole_in += dipole;
      } else {
        if (findInList(iAtom, mpart)) { // HWH: unoptimized
          dipole_in += dipole;
        } else {
          dipole_out += dipole;
        }
    ...
    const double dipole_sq = 2*dipole_in*dipole_out + pow(dipole_in, 2);
    //peLRCone_ += -2.*PI/space()->volume()*dipole_sq;
    en += -2.*PI/space()->volume()*dipole_sq;
  }
  return en;
}
double PairLJCoulEwald::slab_correction_() {
  return slab_correction_(space()->listAtoms());
}

void PairLJCoulEwald::k2maxset(
  const int k2max) {
//  peRecCorrPrefac_ = 0; //SWH
  k2max_ = k2max;
  const int kmax = static_cast<int>(sqrt(k2max)-0.00001)+1;
  kmaxset(kmax, kmax, kmax, k2max_);
}

void PairLJCoulEwald::kmaxset(
    const int kxmax,
    const int kymax,
    const int kzmax,
    const int k2max) {
  ...
  kzmax_ = kzmax;
  kxmax_tot_ = kxmax_ + 1;
  kymax_tot_ = 2*kymax_ + 1;
  kzmax_tot_ = 2*kzmax_ + 1;
  eikrx_.resize(space_->natom()*kxmax_tot_);
  eikry_.resize(space_->natom()*kymax_tot_);
  eikrz_.resize(space_->natom()*kzmax_tot_);
  eikix_.resize(space_->natom()*kxmax_tot_);
  eikiy_.resize(space_->natom()*kymax_tot_);
  eikiz_.resize(space_->natom()*kzmax_tot_);

  if (k2max != 0) {
    k2max_ = k2max;
    gsqmx_ = k2max_*pow(2*PI/space_->minl(), 2);
    gsqmx_ /= 1.00001;
  } else {
    double gsqxmx = pow(2*PI*kxmax_/space_->boxLength(0), 2);
    double gsqymx = pow(2*PI*kymax_/space_->boxLength(1), 2);
    double gsqzmx = pow(2*PI*kzmax_/space_->boxLength(2), 2);
    gsqmx_ = std::max(gsqxmx, gsqymx);
    gsqmx_ = std::max(gsqmx_, gsqzmx);
//  gsqmx_ *= 1.00001;
  }
  ...
  for (int kx = 0; kx <= kxmax_; ++kx) {
    ...
    for (int ky = -kymax_; ky <= kymax_; ++ky) {
      ...
      for (int kz = -kzmax_; kz <= kzmax_; ++kz) {
        ...
        const double k_sq = vecDotProd(kvect, kvect);
        int k2 = kx*kx + ky*ky + kz*kz;
        if ( (k_sq < gsqmx_) && (k2 != 0) ) {  // allen tildesley, srsw
        // if ( (k2 <= k2max_) && (k2 != 0) ) {  // gerhard
          double factor = 1.;
          if (kx != 0) factor = 2;
          kexp_.push_back(2.*PI*factor*exp(-k_sq/4./alpha/alpha)/k_sq
                          /space_->volume());
          k_.push_back(kx);
          k_.push_back(ky+kymax_);
          k_.push_back(kz+kzmax_);
        }
      }
    }
  }
...
void PairLJCoulEwald::addPart() {
  const int nAdd = space_->natom() - eikrx_.size()/kxmax_tot_;
  for (int i = 0; i < nAdd; ++i) {
    int natomprev = eikrx_.size()/kxmax_tot_;
    for (int k = kxmax_tot_ - 1; k >= 0; --k) {
...
//  cout << "multiPartEnerFrr msize " << msize << endl;

  eikrxnew_.clear();
  eikrynew_.clear();
  eikrznew_.clear();
  eikrxnew_.resize(kxmax_tot_*msize);
  eikrynew_.resize(kymax_tot_*msize);
  eikrznew_.resize(kzmax_tot_*msize);
  eikixnew_.clear();
  eikiynew_.clear();
  eikiznew_.clear();
  eikixnew_.resize(kxmax_tot_*msize);
  eikiynew_.resize(kymax_tot_*msize);
  eikiznew_.resize(kzmax_tot_*msize);
  ...
  if (flag == 1 || flag == 3 || flag == 4) {
    ...
    for (int i = 0; i < msize; ++i) {
      ...
      eikrynew_[msize*kymax_+i] = 1.; eikiynew_[msize*kymax_+i] = 0.;
      eikrznew_[msize*kzmax_+i] = 1.; eikiznew_[msize*kzmax_+i] = 0.;
      eikrxnew_[msize+i] = cos(twopilxi*x[dimen_*ipart]);
      eikixnew_[msize+i] = sin(twopilxi*x[dimen_*ipart]);
      eikrynew_[msize*(1+kymax_)+i] = cos(twopilyi*x[dimen_*ipart+1]);
      eikiynew_[msize*(1+kymax_)+i] = sin(twopilyi*x[dimen_*ipart+1]);
      eikrznew_[msize*(1+kzmax_)+i] = cos(twopilzi*x[dimen_*ipart+2]);
      eikiznew_[msize*(1+kzmax_)+i] = sin(twopilzi*x[dimen_*ipart+2]);
      eikrynew_[msize*(-1+kymax_)+i] = eikrynew_[msize*(1+kymax_)+i];
      eikrznew_[msize*(-1+kzmax_)+i] = eikrznew_[msize*(1+kzmax_)+i];
      eikiynew_[msize*(-1+kymax_)+i] = -eikiynew_[msize*(1+kymax_)+i];
      eikiznew_[msize*(-1+kzmax_)+i] = -eikiznew_[msize*(1+kzmax_)+i];
    }
    ...
    for (int ky = 2; ky <= kymax_; ++ky) {
      for (int i = 0; i < msize; ++i) {
        eikrynew_[msize*(ky+kymax_)+i] = eikrynew_[msize*(ky+kymax_-1)+i]
        *eikrynew_[msize*(1+kymax_)+i] - eikiynew_[msize*(ky+kymax_-1)+i]
        *eikiynew_[msize*(1+kymax_)+i];
        eikiynew_[msize*(ky+kymax_)+i] = eikrynew_[msize*(ky+kymax_-1)+i]
        *eikiynew_[msize*(1+kymax_)+i] + eikiynew_[msize*(ky+kymax_-1)+i]
        *eikrynew_[msize*(1+kymax_)+i];
        eikrynew_[msize*(-ky+kymax_)+i] = eikrynew_[msize*(ky+kymax_)+i];
        eikiynew_[msize*(-ky+kymax_)+i] = -eikiynew_[msize*(ky+kymax_)+i];
      }
    }
    for (int kz = 2; kz <= kzmax_; ++kz) {
      for (int i = 0; i < msize; ++i) {
        eikrznew_[msize*(kz+kzmax_)+i] = eikrznew_[msize*(kz+kzmax_-1)+i]
        *eikrznew_[msize*(1+kzmax_)+i] - eikiznew_[msize*(kz+kzmax_-1)+i]
        *eikiznew_[msize*(1+kzmax_)+i];
        eikiznew_[msize*(kz+kzmax_)+i] = eikrznew_[msize*(kz+kzmax_-1)+i]
        *eikiznew_[msize*(1+kzmax_)+i] + eikiznew_[msize*(kz+kzmax_-1)+i]
        *eikrznew_[msize*(1+kzmax_)+i];
        eikrznew_[msize*(-kz+kzmax_)+i] = eikrznew_[msize*(kz+kzmax_)+i];
        eikiznew_[msize*(-kz+kzmax_)+i] = -eikiznew_[msize*(kz+kzmax_)+i];
      }
    }
  }
...
//  cout << "update " << flag << " eikr " << vec2str(eikrx_) << endl;
}
...
void PairLJCoulEwald::initLMPData(const string fileName) {
  ...
  for (int iAtom = 0; iAtom < s->natom(); ++iAtom) {
    ...
    type -= 1;
    qnew[type] = qtmp*chargeConversion_;
...
void PairLJCoulEwald::pairSiteSite_(const int &iSiteType, const int &jSiteType,
  ...
  *neighbor = 1;
  if (epsij != 0) {
    ...
    if (linearShiftFlag_) {
      enlj += peShiftij_[iSiteType][jSiteType];
...
double PairLJCoulEwald::netCharge_(const vector<int> siteList) {
  double netCharge = 0.;
  for (vector<int>::const_iterator iter = siteList.begin();
       iter != siteList.end();
       ++iter) {
    const int iType = space()->type()[*iter];
    netCharge += q_[iType];
  }
  return netCharge;
}

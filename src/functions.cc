...
double volShell(const double rabove, const double rbelow, const int dim) {
  if (dim == 3) {
    ...
    return PI*(rabove*rabove - rbelow*rbelow);
  }
  ASSERT(0, "volShell not implemented for current dimen(" << dim << ")");
  return -1.;
}
...
double closestDistancePointToAxis(const vector<double> &x0,
  const vector<double> &x1, const vector<double> &x2) {
  const int dimen = x0.size();
  ASSERT(x1.size() == x0.size() && x2.size() == x0.size(), "size mismatch");
  vector<double> x01 = x0, x02 = x0, x21 = x0;
  for (int dim = 0; dim < dimen; ++dim) {
    x01[dim] = x0[dim] - x1[dim];
    x02[dim] = x0[dim] - x2[dim];
    x21[dim] = x2[dim] - x1[dim];
  }
  const vector<double> numeratorVec = crossProd(x01, x02);
  return sqrt(vecDotProd(numeratorVec, numeratorVec)) /
         sqrt(vecDotProd(x21, x21));
}

vector<double> closestPointToPointAndAxis(const vector<double> &x0,
  const vector<double> &x1, const vector<double> &x2) {
  const int dimen = x0.size();
  ASSERT(x1.size() == x0.size() && x2.size() == x0.size(), "size mismatch");
  vector<double> x01 = x0, x21 = x0;
  for (int dim = 0; dim < dimen; ++dim) {
    x01[dim] = x0[dim] - x1[dim];
    x21[dim] = x2[dim] - x1[dim];
  }
  const double parametric = vecDotProd(x01, x21)/vecDotProd(x21, x21); 
  vector<double> point(dimen);
  for (int dim = 0; dim < dimen; ++dim) {
    point[dim] = x1[dim] + (x2[dim] - x1[dim])*parametric;
  }
  return point;
}

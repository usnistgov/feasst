...
TEST(CriteriaWLTMMC, mMaxVSmMaxCenter) {
  ...
  try {
    auto c3 = feasst::makeCriteriaWLTMMC(
      {{"beta", "2."},
       {"activ", "1."},
       {"mType", "beta"},
       {"mMinCenter", feasst::str(min)},
       {"mMaxCenter", feasst::str(max)},
       {"nBin", feasst::str(nbin)}});
    CATCH_PHRASE("does not fall within the bins");
  }
}

#ifdef GSL_
TEST(CriteriaWLTMMC, findPeak) {
	feasst::CriteriaWLTMMC criteria("../unittest/colMat");
	criteria.findPeak(150, -4.5);
  EXPECT_NEAR(-3.1731060188673155, log(criteria.activrw()), 1e-7);
}
#endif  // GSL_

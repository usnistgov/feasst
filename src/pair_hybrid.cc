...
double PairHybrid::multiPartEner(
  ...
  if (selected_.size() > 0) {
    ...
    // terminate early if the energy is very large
    int iPair = 0;
    int terminate = 0;
    while ( (iPair < nPairs()) && (terminate == 0) ) {
      pe += pairVec_[iPair]->multiPartEner(mpart, flag);
      if (pe > NUM_INF/10.) {
        terminate = 1;
      }
...
void PairHybrid::accumulate() {
  for (int iPair = 0; iPair < nPairs(); ++iPair) {
    pairVec_[iPair]->accumulate();
  }
}

void PairHybrid::accumulateReset() {
  for (int iPair = 0; iPair < nPairs(); ++iPair) {
    pairVec_[iPair]->accumulateReset();
  }
}

Accumulator PairHybrid::accumulator(const int iPair) const {
  return pairVec_[iPair]->accumulator();
}

...
void WLTMMC::runNumSweeps(const int nSweeps,  //!< target number of sweeps
  ...
  if (window_) {
    ...
    } else if (windowMidPoints_.size() != 0) {
      double min, max;
      if (t == 0) {
        min = c_->bin2m(0);
        max = windowMidPoints_[0];
      } if (t == nWindow_ - 1) {
        min = windowMidPoints_.back();
        max = c_->lastbin2m();
      } else {
        min = windowMidPoints_[t - 1];
        max = windowMidPoints_[t];
      }
      clones[t]->c()->initBins(
        max + binWidth/2.,
        min - binWidth/2.,
        feasstRound((max - min)/binWidth)+1);
    } else if (betaInc_ == 0) {
...
void WLTMMC::runNumSweepsExec_(const int t,    //!< thread
  ...
  //  MPI uses restart files to communicate positions
    ...
    for (int tt = 0; tt < nWindow_; ++tt) {
      stringstream ss;
      ss << rstFileName_ << procFileAppend_ << tt << "criteria";
    ...
    while (allSwept == false) {
      ...
      for (int tt = 0; tt < nWindow_; ++tt) {
        stringstream ss;
        ss << rstFileBaseName_ << procFileAppend_ << tt << "criteria";
...
void WLTMMC::initWindows(const vector<double> midPoints) {
  int nWindow = 1e4;
  #ifdef _OPENMP
    #pragma omp parallel
    {
      if (omp_get_thread_num() == 0) nWindow =  omp_get_num_threads();
    }
  #endif  // _OPENMP
  ASSERT(midPoints.front() > c_->mMin(), "midpoint " << midPoints.front()
    << " is less than minimum macrostate " << c_->mMin());
  ASSERT(midPoints.back() < c_->mMax(), "midpoint " << midPoints.back()
    << " is greater than maximum macrostate " << c_->mMax());

  // check that the midPoints lie in the center of a bin
  //for (int imid = 0; imid < static_cast<int>(midPoints.size()); ++imid) {
  for (vector<double>::const_iterator mid = midPoints.begin();
       mid != midPoints.end();
       ++mid) {
//    const double mid = midPoints.siz
    ASSERT(fabs(c_->bin2m(c_->bin(*mid)) - *mid) < DTOL,
      "midPoint " << *mid << " does not lie in the middle of a bin");
  }
  ASSERT(static_cast<int>(midPoints.size()) + 1 == nWindow,
    "the number of midpoints + 1 must be less than or equal to the number of "
    << "processors available. Or maybe OMP isn't enabled.");
  windowMidPoints_ = midPoints;
  window_ = true;
}

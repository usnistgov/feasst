...
class MC : public BaseRandom {
 public:
  ...
  /// Constructor
  MC(shared_ptr<Space> space, shared_ptr<Pair> pair,
...
//  double boyle(const double tol);
  ...
  // HWH: depreciate this in favor of pair-owned accumulators
  ...
  void setWallClockMaxInHours(const double hours = -1) {
    wallClockMaxInHours_ = hours; }

  /// Initialize all Trials to print verbosely
  void initVerboseTrials();

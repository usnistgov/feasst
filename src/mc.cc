...
void MC::attemptTrial() {
  ...
  /// check walltime for clean termination.
  if (wallClockMaxInHours_ != -1) {
    const double hours = double(clock())/double(CLOCKS_PER_SEC)/60./60.;
    ASSERT(hours < wallClockMaxInHours_,
      "wall clock(" << hours << ") > max(" << wallClockMaxInHours_ << ") reached.");
  }
}

...
#ifndef PAIR_FIELD_SW_H_
#define PAIR_FIELD_SW_H_
 ...
 * \f$ U = \begin{cases}
 *         \infty   & r < \sigma/2
 *         \epsilon & \sigma/2 \leq r \leq rCut
 *         0        & otherwise
 *         \end{cases}\f$
 *
 ...
 * to \f$\sigma/2\f$.
 */
class PairFieldSW : virtual public Pair {
 public:
  /// Constructor
  PairFieldSW(Space * space) : Pair(space) { defaultConstruction_(); }

  PairFieldSW(Space * space, const char* fileName);
 ...
 protected:
  ...
  double sw_energy_(const int &siteType, const double &dist);
};

}  // namespace feasst

#endif  // PAIR_FIELD_SW_H_

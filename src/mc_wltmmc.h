...
class WLTMMC : public MC {
 public:
   ...
   *  For example, the default "_core" would rename files "file" -> "file_core0"
   */
  ...
  /// Initialize OMP windows by the given set of midPoints.
  /// For example, a [0,40]-molecule simulation could be windowed as {10, 30}.
  /// This would result in three windows [0-10], [10-30], [30-40].
  void initWindows(const vector<double> midPoints);
...
//  vector<double> printSat();
 ...
 protected:
  ...
  vector<double> windowMidPoints_;

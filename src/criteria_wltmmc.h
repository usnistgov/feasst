...
class CriteriaWLTMMC : public Criteria {
 public:
  CriteriaWLTMMC(const double beta, //!< \f$\beta = \frac{1}{k_B T}\f$
    /**
  ...
  // Return one of the phases
  CriteriaWLTMMC phaseSplit(const int iPhase) { return phaseSplit(lnPIrw_)[iPhase]; }
...
//  void lnPIenergyIso();
  ...
  /// For reweighting, find global minimum from this many macrostates
  void setNumSmooth(const int nSmooth = 10) { nSmooth_ = nSmooth; }

  /// read-only functions for protected variables
  vector<long double> lnPI() { return lnPI_; }
  vector<double> lnPIdouble() { return vector<double>(lnPI_.begin(), lnPI_.end()); }
  vector<long double> lnPIrw() const { return lnPIrw_; }
  vector<double> lnPIrwdouble() const { return vector<double>(lnPIrw_.begin(), lnPIrw_.end()); }
...
//  vector<double> peMUVT() { lnPIenergyIso(); return peMUVT_; }
  ...
  /// skip every this many macrostates for some analysis
  int skipEvery = 1;

  /// begin on this macrostate for some analysis (to use with skipEvery)
  int firstMacro = 0;
 ...
 protected:
  ...
  static double lnPIrwnmx_wrapper2_(const double lnactivrw, void *params) {
		return static_cast<CriteriaWLTMMC*>(params)->lnPIrwnmx_(lnactivrw);
	}

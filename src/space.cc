...
Space::Space(const char* fileName) {
  ...
  ASSERT(fileExists(fileName),
         "restart file(" << fileName << ") does not exist");
...
//  if ( (!sphereSymMol_) && (natom() != 1) ) qMolInit();
  ...
  // initilize the periodicity
  for (int dim = 0; dim < dimen_; ++dim) {
    std::stringstream ss;
    ss << "periodic" << dim;
    strtmp = fstos(ss.str().c_str(), fileName);
    if (!strtmp.empty()) {
      initPBC(dim, stoi(strtmp));
    }
  }
}
...
void Space::randRotateAboutAxis(const vector<int> &mpart,
  ...
  if (sphereSymMol_ == false) {
    ...
    double angle = -1.;
    if (useMaxRotAsAngle == 0) {
      angle = maxRot*(uniformRanNum() - 0.5);
    } else if (useMaxRotAsAngle == 1) {
      angle = maxRot;
    } else {
      ASSERT(0, "unrecognized useMaxRotAsAngle: " << useMaxRotAsAngle);
    }
    angle += pbc2d(angle);
...
void Space::addMol(const char* type) {
  ...
  ASSERT(mpart.size() > 0, "size error");
  wrap(mpart);
...
void Space::addMolInit(const char* fileName   //!< data file for molecule
  ...
  ASSERT(fileExists(fileName),
         "addMolInit(fileName = " << fileName << ") does not exist.");
...
void Space::updateCells(const double dCellMin, const double rCut) {
  ASSERT(fabs(dCellMin) > feasst::DTOL, "Cell list minimum size, dCellMin("
    << dCellMin << ") is too small. Perhaps updateCells needs to be provided "
    << "an explicit value for dCellMin.");
  ASSERT(dCellMin > 0, "Cell list minimum size must be positive");
  if (dCellMin >= rCut) {
    cellType_ = 1;
  } else {
    cellType_ = 2;
...
int Space::checkCellList() {
  ...
  if (atomCut_) {
    ASSERT(static_cast<int>(atom2cell_.size()) == natom(),
      "atom2cell size(" << atom2cell_.size() << ") does not match number"
      << "of atoms(" << natom() << ")");
  } else {
    ASSERT(static_cast<int>(mol2cell_.size()) == nMol(),
    "mol2cell size(" << mol2cell_.size() << ") does not match number of"
    << "molecules(" << nMol() << ")");
    for (unsigned int i = 0; i < mol2cell_.size(); ++i) {
      ASSERT(mol2cell_[i] < nCell_, "mol2cell_[" << i << " has cell("
        << mol2cell_[i] << ") that does not exist");
  ...
  if (atomCut_) {
    for (unsigned int i = 0; i < atom2cell_.size(); ++i) {
      ASSERT(atom2cell_[i] == atom2cell[i], "old atom2cell[" << i << "]="
        << atom2cell[i] << " does not match new atom2cell[" << i << "]="
    ...
    for (unsigned int i = 0; i < mol2cell_.size(); ++i) {
      ASSERT(mol2cell_[i] == mol2cell[i], "old mol2cell[" << i << "]="
        << mol2cell[i] << " does not match new mol2cell[" << i << "]="
        << mol2cell_[i] << "].");
    }
  }
  for (unsigned int i = 0; i < cellList_.size(); ++i) {
    ...
    for (unsigned int j = 0; j < cellList_[i].size(); ++j) {
      ASSERT(cellList_[i][j] == cellList[i][j], "old cellList[" << i
        << "][" << j << "]=" << cellList[i][j]
        << " does not match new cellList[" << i << "][" << j << "]="
        << cellList_[i][j] << "].");
    }
  }
  return cellMatch;
...
void Space::initLMPData(const std::string fileName, const int nTypesExist) {
  ...
  if (static_cast<int>(bondList_.size()) != 0) {
    ...
    for (int i = 0; i < static_cast<int>(bondList_.size()); ++i) {
      ...
      vector<int> b;
      ASSERT(iType - 1 < static_cast<int>(bondParam_.size()),
        "While reading " << fileName << ", a bond type(" << iType <<
        ") was specified although only " << bondParam_.size() <<
        " bonds exist.");
      b.push_back(iType - 1);
      b.push_back(a1 - 1);
      b.push_back(a2 - 1);
...
int Space::checkSizes() {
  ...
  if (static_cast<int>(type_.size()) != natom()) {
    er = true;
    ermesg << "type(" << type_.size() << ") does not match natom("
           << natom() << ")." << endl;
  }
  if (static_cast<int>(mol_.size()) != natom()) {
    er = true;
    ermesg << "mol(" << mol_.size() << ") does not match natom(" << natom()
           << ")." << endl;
  }
  if (static_cast<int>(listAtoms_.size()) != natom()) {
    er = true;
    ermesg << "listAtoms(" << listAtoms_.size() << ") does not match natom("
           << natom() << ")." << endl;
  }
  ...
  if (static_cast<int>(moltype_.size()) != nMol()) {
    er = true;
    ermesg << "moltype(" << moltype_.size() << ") does not match nMol("
           << nMol() << ")." << endl;
  }
  if (static_cast<int>(listMols_.size()) != nMol()) {
    er = true;
    ermesg << "listMols(" << listMols_.size() << ") does not match nMol("
           << nMol() << ")." << endl;
  }
  if (sphereSymMol_ == false) {
    if (static_cast<int>(qMol_.size())/qdim_ != nMol()) {
      er = true;
      ermesg << "qMol(" << qMol_.size()/qdim_ << ") does not match nMol("
             << nMol() << ")." << endl;
    }
    if (static_cast<int>(xMolRef_.size()) != nMol()) {
      er = true;
      ermesg << "xMolRef(" << xMolRef_.size() << ") does not match nMol("
             << nMol() << ")." << endl;
    }
  } else {
  ...
  if (cellType_ > 0) {
    if (atomCut_) {
      if (static_cast<int>(atom2cell_.size()) != natom()) {
        er = true;
        ermesg << "atom2cell(" << atom2cell_.size() << ") does not match"
               << "natom(" << natom() << ")." << endl;
      }
      int natoms = 0;
      for (unsigned int i = 0; i < cellList_.size(); ++i) {
      ...
      if (natoms != natom()) {
        er = true;
        ermesg << "cellList(" << natoms << ") does not match natom("
               << natom() << ")." << endl;
      }
    } else {
      if (static_cast<int>(mol2cell_.size()) != nMol()) {
        er = true;
        ermesg << "mol2cell(" << mol2cell_.size() << ") does not match nMol("
               << nMol() << ")." << endl;
      }
      int natoms = 0;
      for (unsigned int i = 0; i < cellList_.size(); ++i) {
      ...
      if (natoms != nMol()) {
        er = true;
        ermesg << "cellList(" << natoms << ") does not match nMol("
               << nMol() << ")." << endl;
      }
    }
    if (cellType_ == 1) {
...
int Space::findAddMolListIndex(const string typeStr) const {
  bool match = false;
  for (unsigned int iaml = 0; iaml < addMolList_.size(); ++iaml) {
    if (addMolListType_[iaml].compare(typeStr) == 0) {
      return iaml;
...
void Space::writeRestart(const char* fileName) {
  ...
  for (int dim = 0; dim < dimen_; ++dim) {
    if (periodic_[dim] != 1) {
      file << "# periodic" << dim << " " << periodic_[dim] << endl;
    }
...
void Space::swapPositions(Space *space) {
  ASSERT(natom() == space->natom(), "natom(" << natom() << ") of space id "
    << id_ << " does not match natom(" << space->natom() << ") of space id "
    << space->id() << ")");
  ASSERT(dimen_ == space->dimen(), "dimen(" << dimen() << ") of space id "
    << id_ << " does not match dimen(" << space->dimen() << ") of space id "
    << space->id() << ")");
  vector<double> x = x_;
...
vector<vector<int> > Space::listBondsByMol(const int iMol) const {
  vector<vector<int> > list;
  for (int iAtom = mol2part_[iMol]; iAtom < mol2part_[iMol + 1]; ++iAtom) {
    const vector<vector<int> > alist = listBonds(iAtom);
    list.insert(list.end(), alist.begin(), alist.end());
  }
  return list;
}
...
void Space::pointReflect(const int iSite, const vector<double> &r) {
  for (int dim = 0; dim < dimen_; ++dim) {
    x_[dimen_*iSite+dim] = 2*r[dim] - x_[dimen_*iSite+dim];
  }
}

void Space::pointReflectMol(const int iMol, const vector<double> &r) {
  for (int iSite = mol2part_[iMol]; iSite < mol2part_[iMol+1]; ++iSite) {
    pointReflect(iSite, r);
  }
...
void Space::lineReflectMol(const int iMol, const vector<double> &r1,
  const vector<double> &r2) {
  for (int iSite = mol2part_[iMol]; iSite < mol2part_[iMol+1]; ++iSite) {
    const vector<double> point =
      closestPointToPointAndAxis(x(iSite), r1, r2);
    pointReflect(iSite, point);
  }
...
void Space::initPBC(const int dimen, const int flag) {
  ASSERT(dimen >= 0 && dimen < dimen_,
    "Unrecognized dimen(" << dimen << ")");
  ASSERT(flag == 0 || flag == 1, "flag(" << flag << ") must be 0 or 1");
  periodic_[dimen] = flag;
}
...
vector<double> Space::x(const int iSite) const {
  vector<double> pos(dimen_);
  for (int dim = 0; dim < dimen_; ++dim) {
    pos[dim] = x(iSite, dim);
  }
  return pos;
...
void Space::randLineReflectMol(const int iMol, const double maxMoveParam) {
  const int iSite0 = mol2part_[iMol];
  // eventually use maxMoveParam to determine maximum distance of first point
  // and first site, and/or location of the axis
  ASSERT(fabs(maxMoveParam) < DTOL, "maxMoveParam not implemented");
  const vector<double> x1 = x(iSite0);
  vector<double> x2 = ranUnitSphere(dimen_);
  for (int dim = 0; dim < dimen_; ++dim) {
    x2[dim] += x1[dim];
  }
  lineReflectMol(iMol, x1, x2);
  wrap(imol2mpart(iMol));
}

void Space::randLineReflectMolByBond(const int iMol) {
  const vector<vector<int> > bonds = listBondsByMol(iMol);
  ASSERT(bonds.size() > 1, "reflect by bond requires bonds");
  // pick a random bond
  const vector<int> bond = bonds[uniformRanNum(0, bonds.size() - 1)];
  // reflect iMol by the two positions of the atoms in the bond
  vector<double> x1(dimen_), x2 = x1;
  for (int dim = 0; dim < dimen_; ++dim) {
    x1[dim] = x(bond[1], dim);
    x2[dim] = x(bond[2], dim);
  }
...
int Space::nMolOfType(const std::string molType, int * molIndex) const {
  *molIndex = findAddMolListIndex(molType);
  if (*molIndex == -1) {
    return 0;
  }
  return nMolType()[*molIndex];
}

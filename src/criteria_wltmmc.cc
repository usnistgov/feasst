...
CriteriaWLTMMC::CriteriaWLTMMC(const double beta, const argtype &args)
  ...
  // if mMax wasn't given, parse mMaxCenter
    ...
    ASSERT(cMax > cMin, "cMax(" << cMax <<") must be greater than cMin("
                        << cMin << ")");
    nBin_ = argparse_.key("nBin").integer();
    ASSERT(nBin_ > 1, "no point in WLTMMC when there is only one macrostate"
      << " bin. nBin == " << nBin_);
  ...
  // Error checks
  if (mType_ == "beta") {
    ASSERT(fabs(beta - bin2m(bin(beta))) < DTOL,
      "The initial value of beta(" << beta << ") does not fall within " <<
      "the bins");
  }
...
//  cout << "b " << mNewBin << " a " << acceptFlag << " h " << vec2str(h_) << endl;
  if (collect_) {
    ...
    if (cTripleBanded_) {
      ...
      if (mOldBin == mNewBin) {
        ...
        if ( (tmmc_ == true) && (acceptFlag == 1) ) {
          if (skipEvery != 1 || firstMacro != 0) {
            int dMacro = mNewBin - firstMacro;
            if (dMacro >= 0 && mNewBin % skipEvery == 0) {
              if (mNewBin != lastMacroSubset_) {
                lastMacroSubset_ = mNewBin;
                ++h_[mNewBin];
              }
            }
          } else {
...
void CriteriaWLTMMC::lnPIupdate() {
  if (tmmc_) {
    ...
    vector<int> visits;
    if (skipEvery != 1 || firstMacro != 0) {
      for (int iMacro = 0; iMacro < nBin(); ++iMacro) {
        int dMacro = iMacro - firstMacro;
        if (dMacro >= 0 && iMacro % skipEvery == 0) {
          visits.push_back(h_[iMacro]);
        }
      }
    } else {
      visits = h_;
    }
    if (*std::min_element(visits.begin(), visits.end()) >= nSweepVisPerBin_) {
      ++nSweep_;
      std::fill(h_.begin(), h_.end(), 0);
    }
  }
...
void CriteriaWLTMMC::findPeak(const double nMolPeak,
  ...
  double m = lnactivGuess, m_expected = M_PI;
  double a = -80.0, b = 10.0;
  gsl_function F;

	F.function = &CriteriaWLTMMC::lnPIrwnmx_wrapper2_;
  F.params = this;	// hacky solution from https://stackoverflow.com/questions/10593726/a-function-pointer-issue-how-to-efficiently-interface-with-c-api-ie-gsl-from
	nMolPeak_ = nMolPeak;
...
//	cout << "lnpiav " << lnPIaverage(lnPIrw_) << " peak " << nMolPeak_ << " diff " << pow(lnPIaverage(lnPIrw_) - nMolPeak_, 2) << endl;

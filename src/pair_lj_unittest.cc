...
TEST(PairLJ, periodicity) {
  auto space = feasst::makeSpace({{"dimen", "3"}, {"boxLength", "8"}});
  auto pair = feasst::makePairLJ(space, {{"rCut", "3"}, {"cutType", "linearShift"}});
  vector<double> x(space->dimen(), 0.);
  x[0] = 3.9;
  pair->addMol(x);
  x[0] = -3.9;
  pair->addMol(x);
  pair->initEnergy();
  EXPECT_NEAR(pair->peTot(), 976500000.03611219, 1e-8);
  space->initPBC(0, 0);
  pair->initEnergy();
  EXPECT_NEAR(pair->peTot(), 0, feasst::DTOL);
}

...
void Trial::attempt() {
  ...
  de_ = 0.;
  def_ = 0.;
  WARN(verbose_ == 1, "attempting " << className_ << " " << MAX_PRECISION
    << pair_->peTot());
  attempt1_();
}
...
void Trial::trialMoveDecide_(const double def,
  ...
  // if preFac is zero, still call criteria for WLTMMC to store old state
    ...
    WARN(verbose_ == 1, "move rejected because 0 prefactor " << de_);
    reject_ = 1;
...
bool Trial::equiMolarCheckFails_(const int iMol, const int flag) {
  if (space()->equiMolar() >= 1) {
    //cout << "iMol " << iMol << endl;
    const int iMolType = space()->molid()[iMol];
    if (space()->equiMolar() == 1) {
      const int nimt = space()->nMolType()[iMolType];
      // reject if any molecule is about to be present in quantity 2 greater
      for (int imt = 0; imt < space()->nMolTypes(); ++imt) {
        if (imt != iMolType) {
          if (flag == 1) {
            if (nimt < space()->nMolType()[imt]) {
              return true;
            }
          }
          if (flag == 0) {
            if (nimt > space()->nMolType()[imt] + 1) {
              return true;
            }
          }
        }
    ...
    } else if (space()->equiMolar() == 4) {
      // enforce all species equal
      const int nimt = space()->nMolType()[iMolType];
      for (int imt = 0; imt < space()->nMolTypes(); ++imt) {
        if (imt != iMolType) {
          if (nimt != space()->nMolType()[imt]) {
    ...
    } else if (space()->equiMolar() == 5) {
      int n0 = space()->nMolType()[0];
      int n1 = space()->nMolType()[1];
      if (flag == 0) {
        if (abs(2*n0 - n1) > 1) {
          return true;
        }
        ...
        if (iMolType == 0) {
          ...
          ASSERT(0, "equiMolar flag 5 hard coded for 2 types");
        }
        if (abs(2*n0 - n1) > 1) {
          return true;
        }
      } else {
        ASSERT(0, "unrecognized flag");
      }
    }
  }
  return false;
...
double Trial::confine_volume_() {
  double volume = 1.;
  int dim = 0;
  while (dim < space()->dimen()) {
    if (dim == confineDim_) {
      volume *= confineUpper_ - confineLower_;
    } else {
      volume *= space()->boxLength(dim);
    }
    ++dim;
  }
  return volume;

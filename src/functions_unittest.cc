...
TEST(Functions, product) {
  ...
  x[2] = 3.5;
  x[3] = 50.;
  EXPECT_NEAR(1.5*2.3*3.5*50., feasst::product(x), 1e-12);
  x[0] = 1;
  x[1] = 2;
  x[2] = 3;
  x[3] = 50;
...
TEST(Functions, volShell) {
  EXPECT_NEAR(1.08787908166548, volShell(1.08, 1., 3), 1e-14);
  EXPECT_NEAR(458.14892864851146, volShell(5, 2.5, 3), 1e-14);
  EXPECT_NEAR(0.522761017557342000, volShell(1.08, 1., 2), feasst::DTOL);
  try {
    volShell(1., 0.5, 1);
    CATCH_PHRASE("volShell not implemented for current dimen");
  }
}

TEST(Functions, closestDistancePointToAxis) {
  { vector<double> x0 = {1., 2., 3.}, x1 = {2., 3., 4.}, x2 = {3., 4., 5.};
    EXPECT_NEAR(feasst::closestDistancePointToAxis(x0, x1, x2), 0., feasst::DTOL);
  }
  { vector<double> x0 = {2.354, -1., 1.}, x1 = {0., -1., 0.}, x2 = {0., -1., 1.};
    EXPECT_NEAR(feasst::closestDistancePointToAxis(x0, x1, x2), 2.354, feasst::DTOL);
  }
}

TEST(Functions, closestPointToPointAndAxis) {
  { vector<double> x0 = {1., 2., 3.}, x1 = {2., 3., 4.}, x2 = {3., 4., 5.};
    const vector<double> a = feasst::closestPointToPointAndAxis(x0, x1, x2);
    for (int dim = 0; dim < static_cast<int>(a.size()); ++dim) {
      EXPECT_NEAR(a[dim], x0[dim], feasst::DTOL);
    }
  }
  { vector<double> x0 = {2.354, -1., 1.}, x1 = {0., -1., 0.}, x2 = {0., -1., 1.};
    const vector<double> a = feasst::closestPointToPointAndAxis(x0, x1, x2);
    for (int dim = 0; dim < static_cast<int>(a.size()); ++dim) {
      EXPECT_NEAR(a[dim], x2[dim], feasst::DTOL);
    }
  }
}

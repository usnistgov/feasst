...
void TrialTransform::defaultConstruction_() {
  ...
  if ( (transType_.compare("translate") == 0) ||
    ...
    targAcceptPer = 0.25;
  } else if (transType_.compare("line-reflect") == 0) {
    if (maxMoveParam == maxMoveParamDefault_) maxMoveParam = 0.;
    targAcceptPer = 0.25;
  } else if (transType_.compare("bond-reflect") == 0) {
    maxMoveFlag = 0;
...
void TrialTransform::attempt1_() {
  WARN(verbose_ == 1, "attempting " << transType_ << " " << pair_->peTot());
  if (space()->nMol() > 0) {
    ...
    // rigid translation or rotation
      ...
      if (reject_ == 1) {
        ...
        WARN(verbose_ == 1, transType_ << " for mpart " << vec2str(mpart_));
        trialMoveRecord_();
        if (transType_.compare("translate") == 0) {
          ...
          const int iMol = space()->mol()[mpart_.front()];
          space()->randLineReflectMol(iMol, maxMoveParam);
        } else if (transType_.compare("bond-reflect") == 0) {
          const int iMol = space()->mol()[mpart_.front()];
          space()->randLineReflectMolByBond(iMol);
...
void TrialTransform::tuneParameters() {
  ...
  if ( (transType_.compare("translate") == 0) ||
    ...
    if (upperLimit == 0.) upperLimit = NUM_INF;
              ...
              (transType_.compare("line-reflect") == 0) ||
              (transType_.compare("zaxis-rotate") == 0) ) {
    upperLimit = 1e1;
  } else if (transType_.compare("bond-reflect") != 0) {

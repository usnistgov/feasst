...
class PairLJCoulEwald : public PairLRC {
 public:
  ...
  void k2maxset(
    ...
    const int k2max);
  void kmaxset(const int kxmax, const int kymax, const int kzmax, const int k2max = 0);
   ...
   *  flag==2, old configuration, preparing to delete (same as 0)
   *  flag==3, just inserted particle
   *  flag==4, update per-particle arrays but not energy changes.
  ...
  // Add perpendicular slab correction.
  void initSlabCorrection(
    // disabled if dim < 0
    const int dim = -1) {
    slab_corr_dim_ = dim;
  }
  ...
  int knum() const { return static_cast<int>(k_.size()); }
 ...
 protected:
  ...
  int kxmax_;               //!< maximum wave vector magnitude cut off
  int kymax_;               //!< maximum wave vector magnitude cut off
  int kzmax_;               //!< maximum wave vector magnitude cut off
  int k2max_;              //!< maximum wave vector squared magnitude cut off
  double gsqmx_;              //!< maximum wave vector squared magnitude cut off
  int kxmax_tot_;              //!< maximum wave vector in particluar dimension
  int kymax_tot_;              //!< maximum wave vector squared magnitude cut off
  int kzmax_tot_;              //!< maximum wave vector squared magnitude cut off
  ...
  virtual void pairSiteSite_(const int &iSiteType, const int &jSiteType,
    double * energy, double * force, int * neighbor, const double &dx,
    const double &dy, const double &dz);

  // Overload to track energy types
  // NOTE HWH: Remove this: not necessary
  virtual double pairLoopSite_(
    const vector<int> &siteList,
    const int noCell = 0);

  double chargeConversion_;

//  /// Net charge terms from doi.org/10.1063/1.476320
//  double peRecCorrPrefac_;  //!< SWH; prefactor for net charge term

  /// Return the net charge of the sites.
  double netCharge_(const vector<int> siteList);

  /// Return the net charge of the system
  double netCharge_() { return netCharge_(space()->listAtoms()); }

  /// slab correction
  int slab_corr_dim_ = -1;
  // Frenkel and Smit, equation 12.4.5, page 318
  // note: for mpart which is a subset of all particles, where N = N_total - mpart
  // U(N+mpart) - U(N) = 2*M_in*M_out + M_in**2
  // if mpart is all atoms, this reduces to M**2
  double slab_correction_(const std::vector<int>& mpart) const;
  double slab_correction_();
};

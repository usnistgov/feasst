...
class Trial : public BaseRandom {
 ...
 protected:
  ...
  int confineDim_;
  double confine_volume_(); //!< return the confined volume
  ...
  // check equiMolar constraint
  bool equiMolarCheckFails_(const int iMol,
    /// flag == 0: check constraint after adding iMol
    /// flag == 1: check constraint before deleting iMol
    const int flag);

...
vector<double> BaseRandom::ranShell(const double rabove, const double rbelow,
  ...
  if (dim == 3) {
    ...
    const double theta = 2*PI*uniformRanNum();
    const double fac = pow(
      uniformRanNum() * (rabove*rabove - rbelow*rbelow) + rbelow*rbelow,
      1./2.);
    x[0] = fac*cos(theta);
    x[1] = fac*sin(theta);
  } else {
    ASSERT(0, "ranShell not implemented for current dimen(" << dim << ")");
  }
  return x;
}
...
vector<double> BaseRandom::ranUnitSphere(const int dim) {
  ...
  if (dim == 3) {
    ...
    ASSERT(0, "only implemented for 2 or 3 dimensions (dim=" << dim << ")");
  }
  return x;
}

...
def dict_to_argparse(dictionary):
    ...
    >>> params = {'run_type': 0, 'queue_flags': ""}
    >>> fstio.dict_to_argparse(params)
    ' --run_type 0 --queue_flags ""'
    """
    rtrn = str()
...
def all_sims_complete(filename, procs_per_job):
    """
    See if each job has a sim_id_file with as many lines as procs for ecah job.
    ...
    False
    >>> all_sims_complete('tests/not_a_file.txt', 1)
    False
    """
    ...
    try:
        ...
        return False
    if len(lines) == procs_per_job:
        return True
    return False

def deprecate_warning(params, old_key, new_key):
    if old_key in params:
        print('feasst.fstio', old_key, 'is deprecated. Use', new_key)
        if new_key in params:
            print('do not use both', new_key, 'and', old_key)
        params[new_key] = params.pop(old_key)

#HWH now autogenerated sim_id_file: filename to write simulation id's for each job for status check,
def slurm_single_job(params):
    """
    Write slurm script for one job.

    :param dict params:
        Must have the following keys:
        procs_per_job: number of processors per job,
    ...
    deprecate_warning(params, old_key='procs_per_node', new_key='procs_per_job')
    deprecate_warning(params, old_key='node', new_key='job')
...
python {script} --run_type 0 --job {job} --queue_id $SLURM_ARRAY_JOB_ID --queue_task $SLURM_ARRAY_TASK_ID
...
def slurm_single_node(params):
    print('feasst.fstio.slurm_single_node is deprecated. Use feasst.fstio.slurm_single_job.')
    slurm_single_job(params)

def server(params, args, sim_job_dependent_params, write_feasst_script):
...
def seed_param_write_run(sim, params, args, sim_job_dependent_params, write_feasst_script):
    reseed(params)
    if sim_job_dependent_params:
        sim_job_dependent_params(params)
...
def run_single(sim, params, args, sim_job_dependent_params, write_feasst_script, post_process):
    ...
    if syscode == 0: # if simulation finishes with no errors, write to sim id file
        with open(params['sim_id_file'], 'a', encoding='utf-8') as file1:
            file1.write(str(sim)+'\n')
        # if all sims are complete in job, then remove sim_id_file
        sim_id_file = prefix_job_to_sim_id_file(params['prefix'], params['job'])
        if all_sims_complete(sim_id_file, params['procs_per_job']):
            os.remove(sim_id_file)
            # if all sim_id_files are removed, then post process
            all_complete = True
            for job in range(params['num_jobs']):
                if Path(prefix_job_to_sim_id_file(params['prefix'], job)).is_file():
                    all_complete = False
            if all_complete and post_process is not None:
                post_process(params)

#        # if all sims are complete, post process or test once (by removing sim id file)
#        if all_sims_complete(prefix_job_to_sim_id_file(params['prefix'], params['job']), params['procs_per_job']):
#            removed = False
#            try:
#                for job in range(params['num_jobs']):
#                    os.remove(prefix_job_to_sim_id_file(params['prefix'], job))
#                removed = True
#            except FileNotFoundError:
#                pass
...
def prefix_job_to_sim_id_file(prefix, job):
    return str(prefix) + 'j' + str(job) + '_sim_ids.txt'

def run_simulations(params, queue_function, args, write_feasst_script=None, client=None, sim_job_dependent_params=None, sim_node_dependent_params=None, post_process=None):
    """
    ...
    :param dict params:
        ...
        job: job index.
    :param function sim_job_dependent_params:
        The name of the function that assigns parameters based on the sim and job.
    ...
    if sim_node_dependent_params:
        ...
        if sim_job_dependent_params:
            print('do not use both sim_node_dependent_params and sim_job_dependent_params')
        sim_job_dependent_params = sim_node_dependent_params
    deprecate_warning(params, old_key='procs_per_node', new_key='procs_per_job')
    deprecate_warning(params, old_key='node', new_key='job')
    deprecate_warning(params, old_key='num_nodes', new_key='num_jobs')
    if args.run_type == 0: # run directly
        #print('getting params')
        params = read_write_params(args, params)
        #if params['num_jobs'] > 1:
        #    print('Warning: requested num_jobs:', params['num_jobs'], ', but running interactively (e.g., --run_type 0 as default) will only run the first job. This means that post_process will return an error if it does not have all the required files.')
        params['sim_id_file'] = prefix_job_to_sim_id_file(params['prefix'], params['job'])
        #print('have params', params['beta'])
        if client is None:
            #print('Run text interface')
            with Pool(params['procs_per_job']) as pool:
                sims = list(split(range(params['num_sims']), params['num_jobs']))[params['job']]
                codes = pool.starmap(run_single, zip(sims, repeat(params), repeat(args),
                                                     repeat(sim_job_dependent_params),
            ...
            for s in sims:
                params['sim'] = s
                #print('s', s)
                proc = multiprocessing.Process(target=server, args=(params, args, sim_job_dependent_params, write_feasst_script))
                proc.start()
        ...
        for job in range(params['num_jobs']):
            params['job'] = job
            params['sim_id_file'] = """{prefix}j{job}_sim_ids.txt""".format(**params)
...
def num_sites_in_fstprt(fstprt_file, feasst_install=""):
    ...
    4
    >>> fstio.num_sites_in_fstprt('/feasst/particle/n-butane.txt', './')
    4
    """
...
def write_if_file_DNE(filename, content):
    """
    Write to a file if that file does not exist. Otherwise, do nothing.

    >>> from feasst import fstio
    >>> file1 = fstio.write_if_file_DNE('does_not_exist', 'content')
    >>> with open('does_not_exist', 'r', encoding='utf-8') as file1:
    ...     file1.readlines()
    ['content']
    >>> file1 = fstio.write_if_file_DNE('tests/gr.csv', 'content')
    >>> with open('tests/gr.csv', 'r', encoding='utf-8') as file1:
    ...     file1.readlines()[1][:5]
    '0.025'
    """

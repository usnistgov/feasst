...
class MacrostateDistribution:
    ...
    """
    def __init__(self, file_name, macrostate_header='state', ln_prob_header='ln_prob', comment="#", integer_only=False):
        """
        ...
        :param bool ignore_expanded:
            Ignore expanded ensemble microstates with non-integer values.
        ...
        if file_name is not None:
            df = pd.read_csv(file_name, comment=comment)
            if integer_only:
                df = df[df['state']%1 < 1e-8]
...
def splice_files(prefix, suffix, ln_prob_header=None, extra_overlap=0, shift=True, integer_only=False):
    """
